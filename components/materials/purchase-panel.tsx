...
export function PurchasePanel({ assetId, priceCents }: { assetId: string; priceCents: number }) {
  ...
  async function addToDraft() {
    ...
    try {
      ...
      if (!response.ok || !payload.data) throw new Error(payload.error?.message ?? "加入待购清单失败。");
      setSaved(true); setOrderId(payload.data.id); setMessage("已保存到服务端待购订单，刷新或换页面后仍可恢复。");
    } catch (error) { setMessage(error instanceof Error ? error.message : "加入待购清单失败。"); }
  ...
  return (
    <div className="border-t border-line pt-5">
      ...
      <button className="ui-button-primary mt-5 w-full" disabled={busy} onClick={addToDraft} type="button"><ShoppingBagOpen aria-hidden="true" size={18} weight="bold" />{busy ? "正在保存…" : "加入待购清单"}</button>
