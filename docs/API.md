...
版本：v3.3
日期：2026-07-19
状态：T008 接口合同已批准；T009 身份基础、T010 公开素材接口已完成；T011 上传者素材提交流程进入 Review
...
T011 已实现提交前校验：至少一个原文件、每个原文件的衍生任务均为 `ready`，人物素材另需至少一个 `person_proof`。提交与重复请求使用 `Idempotency-Key` 复用同一认证费记录；本阶段只设置 `certificationStatus=pending_payment`，`reviewStatus` 继续保持 `draft`，不会伪造支付成功或进入后台审核。
