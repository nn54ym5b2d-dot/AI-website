...
版本：v3.16（2026-09-19 需求统一修订）
日期：2026-09-19
状态：T008 接口合同已批准；T009-T015 已完成；T016 Ready

## 1. 文档定位

本文件保留 T009–T015 既有 Web 核心 API 合同，并在第 16 节列出 PRD v2.3 新版增量要求。接口覆盖用户登录、邀请码激活、素材浏览、素材上传、审核认证、订单支付、授权下载、收益记录、管理后台和外部观察员只读看板。

T015 实现状态：管理员用户角色/状态更新、观察员账号列表/创建/状态变更和第 11 节六个观察员只读接口均已基于 PostgreSQL/Prisma 落地。观察员请求按周期从业务表生成并持久化汇总快照，只返回聚合数据和本人 0% 分成记录；不存在观察员写入、导出或明细接口。
...
公开业务身份规则：新账号默认获得 `buyer`；`POST /api/v1/invites/activate` 成功后必须同时保证 `buyer + uploader` 为 active，不能用上传资格替换购买能力。已有仅 `uploader` 的历史账号由增量 migration 补齐 `buyer`。`GET /api/v1/account/summary` 可继续按同一账号的全部有效角色返回真实聚合，但页面分别由 `/account` 购买者中心和 `/account/uploader` 上传者中心选择展示对应业务字段。
...
本次只确定接口边界，不实现完整 API，不接入真实微信支付、支付宝、腾讯云 COS、短信、邮件、微信登录或权证链存证服务，也不生成正式 OpenAPI 文件。
...
本地测试补充：`GET /api/v1/auth/local-outbox` 只在 local provider 已启用且请求 Host 为本机回环地址时返回最近 20 条验证码投递；响应 `Cache-Control: no-store`，不返回 challenge ID。该接口服务于登录前本地测试，不属于公开或生产 API，非本机访问统一返回 404。
...
身份核验按权证链接入及业务要求确定。真人形象提交形象授权材料，漫剧形象提交创作或合法授权材料；道具和场景按审核要求补充证明。接口新增范围见本文件新版需求增补。
...
本节激活即增加上传角色是既有实现合同，不等于会员已实现；新版需加入第 16 节会员关系与迁移设计。
...
| `GET /api/v1/auth/local-outbox` | 仅本机测试，无需登录 | 无 | 最近 20 条本地投递的方式、标识、验证码、过期时间和过期状态；禁止缓存且不返回 `challengeId` | 非 local provider 或非回环 Host 返回 `RESOURCE_NOT_FOUND` |
...
T014 已将 `sort=popular` 接入真实 `downloads` 数量降序，并使用上架时间、ID 作为稳定次序；未产生下载的素材仍可稳定展示，不再用“最新上架”冒充热门。
...
- 既有字段规则（真人/漫剧增量见第 16 节）：`person_proof` 只允许人物素材且提交前必需；`supporting_proof` 只允许物件/道具和场景素材且为选填。两类证明材料都保持私有；认证证书使用管理员专用上传接口。
...
本地开发环境的短时地址会跳转到 `GET /api/v1/download-bundles/{bundleFileId}/local-test`，并携带到期时间与 HMAC 签名。该路由只在非生产环境启用，仍会校验签名、过期时间和 ZIP 文件归属；生产环境必须由 T017 的私有 COS provider 替换，不能把本地地址当作生产合同。

购买者页面对 `pending`、`failed` 和 `ready` 状态统一提供“下载 ZIP”主动作：首次点击先调用生成接口，返回 `ready` 后自动请求文件下载，不要求用户再次点击；已生成 ZIP 直接进入逐次短时地址签发。`processing` 仅显示正在准备状态。
...
| `GET /api/v1/uploader/revenue/summary` | 有效上传者 | 无 | 有效购买总次数、上传者购买收益，以及按素材汇总的购买次数和收益；退款/撤销记录不计入，金额使用整数分 | `FORBIDDEN` |

收益接口不返回购买者手机号、邮箱、支付账号、完整订单支付流水或其他上传者数据。上传者汇总只改变展示口径；底层初始收益和冲正记录仍保留，供幂等、审计和后台财务使用。
...
| 方法与路径 | 成功返回 |
|---|---|
| `GET /api/v1/observer/dashboard` | 最新周期新增上传、新认证上架、有效购买、下载、已支付订单、交易收益、上传收益和当前观察员分成摘要；交易/上传收益同时返回支付与退回汇总 |
| `GET /api/v1/observer/platform-metrics` | 周期快照和趋势：上传、上架、审核、认证、购买、订单、授权、下载和收益汇总 |
| `GET /api/v1/observer/assets-summary` | 人物、物件/道具、场景的上传、认证上架、有效购买、100% 交易收益和下载汇总 |
| `GET /api/v1/observer/downloads-summary` | 总下载、付费下载、按周期和类型的趋势汇总 |
| `GET /api/v1/observer/revenue-summary` | 素材交易支付、交易退回、100% 交易收益、认证上传费支付/退回、上传收益，以及平台和上传者分成汇总 |
...
- `transactionRevenueCents = transactionRevenuePaidCents - transactionRevenueRefundCents`，按全部素材成交金额的 100% 统计，其中包含上传者和平台分成，不代表平台利润。
- `uploadRevenueCents = uploadFeePaidCents - uploadFeeRefundCents`，只统计已成功支付或已成功退回的认证上传费。
- `purchaseCount` 只统计当前仍有效的购买订单明细；退款或授权撤销后从有效购买数中扣除。
...
5. 为每个订单明细创建一条包含成交时配置比例（默认上传者 80%、平台 20%、观察员 0%）及上传者、平台金额快照的收益记录。
...
## 16. PRD v2.3 接口增量要求（未实现）

原接口表用于描述既有 Web 合同；T018 先确定新增字段及路径，随后按任务实现，不把下列能力写成已经上线的接口。

- 会员：邀请码校验/消费与会员建立保持事务及幂等；本人查询会员信息，超级管理员管理身份和状态并记录审计。会员身份与上传角色分开判断，联动规则确认后实现。
- 素材：列表/详情及上传/审核支持真人、漫剧区分；证明材料校验按类别执行，不对公众或观察员返回私有材料。
- 认证：后台提交固定版本哈希至权证链，查询并核验结果、保存编号和证书凭证；重复请求、超时、失败重试和文件变更有明确状态约束，不重复收费、不绕过审核上架。
- 授权：创建订单时明确买方接受的具体授权范围，支付后按订单明细生成不可变授权快照；用途、AI 限制及认证关联可追溯。期限法律待定，T022 后完善契约与校验。
- 微信小程序：设计微信登录与平台账号绑定、跨端身份校验、支付和上传/下载分工；沿用服务端权限、幂等及私有文件签名边界，不直接复制浏览器 Cookie/CSRF 假定。
- 默认价格及分成沿用旧配置且可后台修改；修改不重算订单、认证费或收益历史快照。
