...
test("T011 上传 API 跑通两套表单规则、幂等完成、独立衍生对象与认证费待支付状态", { skip: !shouldRun }, async () => {
  ...
  const chargeSnapshot = await getPrisma().certificationFeeCharge.findUniqueOrThrow({
    where: { id: submittedData.certificationFeeCharge.id }
  });
  assert.equal(JSON.stringify(chargeSnapshot.settingsSnapshot).includes("certificationFeeCents"), true);
  assert.equal(JSON.stringify(chargeSnapshot.settingsSnapshot).includes("downloadEligibilityDays"), true);
  ...
  const optionalProofIntentResponse = await createUploadIntent(
    jsonRequest(
      `/api/v1/uploader/assets/${objectAsset.id}/file-uploads`,
      "POST",
      {
        fileType: "supporting_proof",
        fileName: "ownership-proof.png",
        mimeType: "image/png",
        sizeBytes: 120_000,
        sha256: "d".repeat(64)
      },
  ...
  const optionalProofIntentPayload = await json<{ uploadId: string }>(optionalProofIntentResponse);
  assert.equal(
    optionalProofIntentResponse.status,
    201,
    optionalProofIntentPayload.error?.message
  );
  const optionalProofIntent = optionalProofIntentPayload.data;
  const optionalProofComplete = await completeUpload(
    jsonRequest(
      `/api/v1/uploader/assets/${objectAsset.id}/file-uploads/${optionalProofIntent.uploadId}/complete`,
      "POST",
      {},
      writeHeaders(uploader)
    ),
    { params: Promise.resolve({ assetId: objectAsset.id, uploadId: optionalProofIntent.uploadId }) }
  );
  assert.equal(optionalProofComplete.status, 200);
  const optionalProofRow = await getPrisma().assetFile.findFirstOrThrow({
    where: { assetId: objectAsset.id, fileType: "supporting_proof" }
  });
  assert.equal(optionalProofRow.accessScope, "private");
