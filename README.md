...
- T011 / Issue #6 已由 Robert 验收并通过 PR #30 压缩合并到 `main`（`a04ca6c`），状态为 `Done`：两套上传表单、素材草稿、上传意图、完成确认、不可变最终 key、独立水印衍生对象元数据、人物必填/物件场景选填证明材料、提交后的待支付结果页、幂等提交和本机一键预览均已跑通。T012 / Issue #7 与 T013 / Issue #8 当前均为 `Ready`，下一优先任务为 T012。
