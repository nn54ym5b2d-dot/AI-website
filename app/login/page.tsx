...
import { isLocalAuthProviderEnabled, isLoopbackHostHeader } from "@/lib/auth/local-outbox";

type LoginPageProps = { searchParams: Promise<{ next?: string; email?: string; method?: string }> };

export default async function LoginPage({ searchParams }: LoginPageProps) {
  const { next, email, method } = await searchParams;
  const requestHeaders = await headers();
  const showLocalOutbox = isLocalAuthProviderEnabled() && isLoopbackHostHeader(requestHeaders.get("host"));
  return (
    <PageShell compact description="使用手机号或微信登录；首次使用将自动创建账号。邮箱登录位于下方，首次使用邮箱仍需验证并绑定手机号。" eyebrow="Account access" title="登录/注册源素库">
      <AuthFlowForm
        initialEmail={email}
        initialMethod={method === "email" ? "email" : method === "wechat" ? "wechat" : "phone"}
        nextPath={next}
        showLocalOutbox={showLocalOutbox}
      />
