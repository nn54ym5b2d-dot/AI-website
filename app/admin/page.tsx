...
export default async function AdminPage() {
  ...
  const dashboard = contentAdmin ? await getAdminDashboard() : null;
  ...
  return (
    <PageShell actions={<SecondaryLink href="/observer">观察员只读入口</SecondaryLink>} description={`当前以 ${access.user.displayName} 登录。所有后台权限继续由服务端按有效子角色检查。`} eyebrow="Operations" title="管理后台">
      <div className="grid gap-8">
        {dashboard ? (
          <>
            <div className="rounded-lg border border-warning/25 bg-amber-50 px-4 py-3 text-xs leading-5 text-warning">T012 审核、认证和上架操作已接入本地 PostgreSQL；认证机构、真实证书文件正文、COS 与支付仍未接入。</div>
