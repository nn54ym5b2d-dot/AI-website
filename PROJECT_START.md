...
- T007 数据库表结构细化版已完成初稿，覆盖用户、素材、认证、订单、支付、退款、授权、下载、收益、后台权限、外部观察员汇总和敏感字段边界，已创建 Draft PR #14，等待 Robert 审核。
...
1. Robert 审核 GitHub Issue #2 / T007 的数据库表结构细化版。
2. T007 审核通过后执行 GitHub Issue #3 / T008，细化核心 API。
...
- T007 数据库表结构是否能覆盖素材、认证、订单、授权、下载、收益和外部观察员汇总。
- 是否确认后续数据库实现采用 Prisma + PostgreSQL，并在复杂索引、视图或统计场景补充原生 SQL migration。
