...
- T012 / Issue #7 已由 Robert 验收并通过 PR #32 压缩合并到 `main`（`8e5b42b`），状态为 Done；T012A / Issue #33 已完成页面验收并通过 PR #35 压缩合并到 `main`（`9238226`），Issue 已关闭，状态为 Done；T013 已解锁为 Ready，T014-T016 继续 Blocked，T017 保持 Backlog。
...
- T013 / Issue #8 已完成本地实现并进入 Review：支持多素材服务端订单、系统设置价格与不可变成交快照、认证费/素材购买测试支付、签名与金额校验、回调幂等、逐明细授权/下载资格/初始收益、完整订单明细退款与认证费退款，以及购买/授权个人页和订单/支付/退款/授权后台。隔离 PostgreSQL 自动化测试 22/22、lint、typecheck、build、桌面和 390×844 浏览器闭环及草稿 PR #37 远程 `verify` 均已通过；真实微信支付/支付宝仍未接入。
...
- Notion 总控页已按 v5.7 同步 T013 Review、草稿 PR #37、检查证据和后续任务边界，并在写入后重新读取确认。
...
1. Robert 预览并验收 T013 / Issue #8；草稿 PR #37 远程 `verify` 已通过，验收后合并 PR，再将 T013 转为 Done。
2. T013 合并后解锁 T014，继续实现私有 ZIP 下载和收益展示；真实支付和其他生产外部服务仍统一留在 T017。

## 5. 当前最高优先级

当前最高优先级是验收 GitHub Issue #8 / T013 `开发订单、支付测试流程和授权记录`。本地实现和检查已完成，状态为 Review；在 Robert 验收、PR 合并前不转 Done，也不提前解锁 T014。真实外部接入继续统一在 T017 完成。
