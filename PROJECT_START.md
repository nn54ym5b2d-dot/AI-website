...
- Notion 总控页已按 v6.4 同步 T014 Done、PR #39 合并、Issue #9 关闭、T015 Ready 和后续任务边界，补齐 T014 ZIP 下载与收益记录明细，并在写入后重新读取确认。
...
当前最高优先级是 GitHub Issue #11 / T016 `MVP 全流程测试和修正`，状态为 Ready，等待 Robert 明确开始指令。真实外部接入继续统一在 T017 完成。
