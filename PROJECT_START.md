...
- T012 / Issue #7 已由 Robert 验收并通过 PR #32 压缩合并到 `main`（`8e5b42b`），状态为 Done；T012A / Issue #33 已完成页面验收并通过 PR #35 压缩合并到 `main`（`9238226`），状态为 Done；T013 / Issue #8 已通过 PR #37 压缩合并到 `main`（`e524f9e`），状态为 Done；T014 / Issue #9 已完成本地实现并进入 In Progress，T015-T016 继续 Blocked，T017 保持 Backlog。
...
- T014 / Issue #9 已在 `codex/t014-download-revenue` 完成本地实现：按授权时不可变原文件清单生成真实 ZIP，逐次签发短时地址并记录下载；退款或管理员撤销会撤销入口并幂等冲正收益；购买者、上传者和后台专用页面以及真实下载次数热门排序已接入 PostgreSQL。当前使用严格标记的本机私有存储 provider，真实 COS 仍属于 T017。

当前未完成：

- T014 已完成本地代码、migration、自动化和响应式页面实现，正在进入 PR/远程检查与 Robert 验收；在合并前保持 In Progress/Review，不提前标记 Done。
...
- Notion 总控页已按 v5.8 同步 T013 Done、PR #37 合并、Issue #8 关闭、T014 Ready 和后续任务边界，并在写入后重新读取确认。
...
当前最高优先级是 GitHub Issue #9 / T014 `开发 ZIP 下载和收益记录`，本地实现已完成，下一步是草稿 PR、远程检查与 Robert 验收。真实外部接入继续统一在 T017 完成。
