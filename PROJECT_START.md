...
- T010 当前等待 Robert 在草稿 PR 中验收，尚未标记为 Done；T011 继续保持 Blocked，等 T010 验收并合并后再解锁。
...
1. Robert 验收 T010 素材浏览、筛选、详情和水印预览闭环；验收通过并合并后把 T010 转为 Done。
2. T010 合并后解锁 T011；同时可准备腾讯云账号、COS、备案资料和部署环境，但真实外部接入仍统一在 T017 完成。

## 5. 当前最高优先级

当前最高优先级是验收 GitHub Issue #5 / T010 `开发素材浏览和详情页`。T010 已进入 Review，但在 Robert 验收和 PR 合并前不标记为 Done，也不提前解锁 T011；真实外部接入仍统一在 T017 完成。
