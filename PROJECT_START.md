...
- T012 / Issue #7 已由 Robert 验收并通过 PR #32 压缩合并到 `main`（`8e5b42b`），状态为 Done；T012A / Issue #33 已完成页面验收并通过 PR #35 压缩合并到 `main`（`9238226`），状态为 Done；T013 / Issue #8 已通过 PR #37 压缩合并到 `main`（`e524f9e`），状态为 Done；T014 / Issue #9 已由 Robert 验收并通过 PR #39 压缩合并到 `main`（`aa42ad5`），状态为 Done；T015 本地实现与检查已完成并转为 Review，T016 继续 Blocked，T017 保持 Backlog。
...
- T015 / Issue #10 已实现受约束的用户角色/状态管理、最后一名有效超级管理员保护、观察员账号生命周期、六个只读聚合接口与真实 `/observer` 看板；后续补充仅限本机回环地址访问的验证码箱、四个权限测试账号入口和登录预填。观察员看板已按 Robert 复核改为“新认证上架”、有效购买次数、100% 素材交易净成交额口径的“交易收益”和认证上传费净额口径的“上传收益”，并将这两项收益作为顶部前两张指标卡；下方收入汇总只展示交易收益、上传收益和二者相加的收入合计。两张收益卡可展开收支并查看计算说明，分类汇总同步增加购买与交易收益。隔离 PostgreSQL 自动化测试 28/28、lint、typecheck、生产构建及桌面/390×844 浏览器复核均通过，草稿 PR #42 已创建，当前等待 Robert 验收。
- GitHub Project #1 已重新读取确认 T015 Review、T016 Blocked、T017 Backlog；Notion 总控页 v6.10 已同步观察员收入汇总公式及验收证据，并在写入后重新读取确认。

当前未完成：

- T015 / Issue #10 当前为 Review；尚未获得 Robert 页面验收，也尚未合并到 `main`，不能提前标记 Done 或解锁 T016。
...
1. 由 Robert 验收 T015 / Issue #10 的管理员权限与观察员只读页面；验收并合并前保持 Review。
2. 真实支付、COS、图片处理、CDN 和其他生产外部服务仍统一留在 T017，不在 T015 中伪装接入。

## 5. 当前最高优先级

当前最高优先级是 GitHub Issue #10 / T015 `开发后台权限和外部观察员只读视图`，状态为 Review，等待 Robert 验收。真实外部接入继续统一在 T017 完成。
