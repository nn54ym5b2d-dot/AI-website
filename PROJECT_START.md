...
- GitHub Project #1 上次复核状态为：T006-T011、T009A Done，T012、T013 Ready，T014-T016 Blocked，T017 Backlog；本轮 T012 已进入 Review，但因本机 `gh` 令牌失效，Project 卡片仍待同步。
...
- T012 / Issue #7 已在 `codex/t012-admin-review` 完成本地实现并进入 Review：运营/超级管理员可审核素材、编辑受限基础信息、维护认证记录、受控查看敏感文件、上架/下架并查询审计日志；财务管理员和外部观察员均由服务端拒绝。认证证书和凭证当前继续使用 `local_test` 元数据 provider，不代表真实文件或政府认证网站已接入。
