...
- T013 / Issue #8 已完成本地实现并进入 Review：支持多素材服务端订单、系统设置价格与不可变成交快照、认证费/素材购买测试支付、签名与金额校验、回调幂等、逐明细授权/下载资格/初始收益、完整订单明细退款与认证费退款，以及购买/授权个人页和订单/支付/退款/授权后台。隔离 PostgreSQL 自动化测试 22/22、lint、typecheck、build、桌面和 390×844 浏览器闭环均已通过；真实微信支付/支付宝仍未接入。
...
- Notion 总控页已按 v5.6 同步 T012A Done、PR #35 合并证据与 T013 Ready，并在写入后重新读取确认。
...
1. Robert 预览并验收 T013 / Issue #8；远程 `verify` 通过后合并 PR，再将 T013 转为 Done。
