...
- GitHub 应用连接仍可用于仓库、Issue、PR 和 Project 同步；本机 `gh` 当前令牌已失效，后续需单独重新登录。Git 推送继续走独立的 macOS 钥匙串认证；任何 token 都不得写入项目文件或聊天。
- GitHub Project #1 已重新读取确认：T006-T011、T009A Done，T012 Review，T013 Ready，T014-T016 Blocked，T017 Backlog；T012 草稿 PR #32 已创建。
...
- T012 / Issue #7 已在 `codex/t012-admin-review` 完成本地实现并通过草稿 PR #32 进入 Review：运营/超级管理员可审核素材、编辑受限基础信息、维护认证记录、受控查看敏感文件、上架/下架并查询审计日志；财务管理员和外部观察员均由服务端拒绝。认证证书和凭证当前继续使用 `local_test` 元数据 provider，不代表真实文件或政府认证网站已接入。
- T012 新增的数据库 migration 已应用到本地开发库；隔离 PostgreSQL 完整测试 19/19、lint、typecheck 和生产构建均通过。应用内浏览器在首次连接失败后被安全策略阻止再次访问 localhost，因此本轮没有把自动化视觉检查伪装为已完成，需由 Robert 使用本机预览复核页面。
- T012 草稿 PR #32 的首次 `verify` 暴露测试文件并发共享数据库的竞态；已将数据库集成测试改为串行并在 T012 用例结束后撤销临时上架状态。修复提交 `73518ab` 的 GitHub Actions `verify` 已通过。

当前未完成：

- 交易、下载和收益功能尚未正式开发；后续任务必须延续本地真实 PostgreSQL/Prisma、可替换 provider 和自动化测试，不得回退到无法运行的 mock。
- T012 当前为 Review，等待 Robert 本机页面验收、PR 检查和合并；T013 继续保持 Ready，但在 T012 验收收口前不切换当前任务。
- Notion 总控页已按 T012 Review、本地检查结果和外部服务边界同步，并在写入后重新读取确认。
...
1. Robert 使用本机预览验收 T012 管理后台；确认后完成 PR、Issue 和 Project 收口。
2. T012 完成后执行 T013 / Issue #8；真实支付和外部服务接入分别按 T013、T017 的既定边界推进。

## 5. 当前最高优先级

当前最高优先级是验收 GitHub Issue #7 / T012 `开发后台素材审核和认证记录`。本地功能与自动化检查已完成，当前为 Review；T013 仍为 Ready，真实外部接入继续统一在 T017 完成。
