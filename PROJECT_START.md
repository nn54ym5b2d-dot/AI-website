...
1. 开始 T013 / Issue #8：开发订单、支付测试流程和授权记录，并继续使用 T012A 已落地的系统设置与不可变业务快照。
2. 真实支付和外部服务接入分别按 T013、T017 的既定边界推进，不把测试 provider 描述为生产接入。

## 5. 当前最高优先级

当前最高优先级是 GitHub Issue #8 / T013 `开发订单、支付测试流程和授权记录`。T012A 已完成并解除其前置阻塞；真实外部接入继续统一在 T017 完成。
