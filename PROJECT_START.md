...
- T015 / Issue #10 已实现受约束的用户角色/状态管理、最后一名有效超级管理员保护、观察员账号生命周期、六个只读聚合接口与真实 `/observer` 看板；后续补充仅限本机回环地址访问的验证码箱、四个权限测试账号入口和登录预填。观察员看板已按 Robert 复核改为“新认证上架”、有效购买次数、100% 素材交易净成交额口径的“交易收益”和认证上传费净额口径的“上传收益”，两张收益卡可展开收支并查看计算说明，分类汇总同步增加购买与交易收益。隔离 PostgreSQL 自动化测试 28/28、lint、typecheck、生产构建及桌面/390×844 浏览器复核均通过，草稿 PR #42 已创建，当前等待 Robert 验收。
