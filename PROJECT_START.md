...
- T014 / Issue #9 已在 `codex/t014-download-revenue` 完成本地实现：按授权时不可变原文件清单生成真实 ZIP，逐次签发短时地址并记录下载；退款或管理员撤销会撤销入口并幂等冲正收益；上传资格固定为 `buyer + uploader`，`/account` 购买者中心与 `/account/uploader` 上传者中心独立展示各自业务，三个中心切换/上传主操作统一使用品牌红；购买者、上传者和后台专用页面以及真实下载次数热门排序已接入 PostgreSQL。当前使用严格标记的本机私有存储 provider，真实 COS 仍属于 T017。

当前未完成：

- T014 已完成本地代码、双角色补齐 migration 和 24/24 自动化；本轮品牌红按钮调整的 lint、typecheck、build、桌面/390×844 响应式复核及提交 `265ac5b` 的草稿 PR #39 远程 `verify` 均已通过，在 Robert 验收与合并前保持 Review，不提前标记 Done。
...
- Notion 总控页已按 v6.2 同步 T014 双角色、双业务中心、三个品牌红主操作、草稿 PR #39 和后续任务边界，并在写入后重新读取确认。
