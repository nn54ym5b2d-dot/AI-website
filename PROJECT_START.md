...
- T009 已解除 T008 阻塞并调整为 Ready，尚未正式开始开发。
...
1. 执行 GitHub Issue #4 / T009，使用 Docker Compose + PostgreSQL 16 + Prisma 开发用户注册登录和角色基础。
2. 同步准备腾讯云账号、腾讯云 COS、腾讯云备案资料和部署环境。

## 5. 当前最高优先级

当前最高优先级是执行 T009：建立可重复启动的本地 PostgreSQL 16、Prisma schema、migration、测试数据、注册登录和角色基础。腾讯云账号、COS、支付和备案可以并行准备，但不阻塞 T009-T016 的本地真实实现；这些外部接入统一在 T017 完成。
