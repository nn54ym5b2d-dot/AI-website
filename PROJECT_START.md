...
- T008 核心 API 审核修订版已完成，覆盖 `/api/v1`、Cookie/CSRF、角色权限、COS 直传、支付回调、完整订单明细退款、ZIP 下载、条款接受、收益、后台和观察员汇总，草稿 PR #15 等待 Robert 最终审核。
- Robert 已确认：素材购买只能按完整订单明细退款；多角度原文件打包 ZIP 下载；实名认证延期到 MVP 后；只有人物素材要求必要证明材料。

当前未完成：

- 核心业务功能尚未正式开发；T009 起必须使用本地真实 PostgreSQL/Prisma、可替换测试 provider 和自动化测试，不再只做 mock。
- T008 尚未获得 Robert 最终审核并合并；T009 仍保持 Blocked。
...
1. Robert 审核本次全项目修订并合并 GitHub Issue #3 / T008 对应的 PR #15。
2. 审核通过后执行 GitHub Issue #4 / T009，开发用户注册登录和角色基础。
3. 同步准备腾讯云账号、腾讯云 COS、腾讯云备案资料和部署环境。

## 5. 当前最高优先级

当前最高优先级是 Robert 最终审核 T008 修订版，确认后合并 PR #15 并把 T009 调整为 Ready。腾讯云账号、COS、支付和备案可以并行准备，但不阻塞 T009-T016 的本地真实实现；这些外部接入统一在 T017 完成。
