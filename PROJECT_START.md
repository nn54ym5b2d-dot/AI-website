...
- GitHub Project #1 已于 2026-07-19 重新读取确认：T006-T009、T009A 为 Done，T010 为 Ready，T011-T016 为 Blocked，T017 为 Backlog，与本地任务清单一致。
...
- T010 尚未开始功能开发，当前已解除 T009A 阻塞并进入 Ready；启动前读取 Issue #5、已确认视觉基线和水印衍生图边界。T010 使用本地种子/受控适配器提供已生成水印预览，不把真实腾讯云服务描述为已接通。
...
1. 读取 GitHub Issue #5 和相关 docs，拆解并执行 T010：素材浏览和详情页。
2. 并行准备腾讯云账号、COS、备案资料和部署环境，但真实外部接入仍统一在 T017 完成。

## 5. 当前最高优先级

当前最高优先级是 GitHub Issue #5 / T010 `开发素材浏览和详情页`。T009A 已完成并成为 T010-T016 的视觉基线；腾讯云账号、COS、支付和备案可以并行准备，但真实外部接入统一在 T017 完成。
