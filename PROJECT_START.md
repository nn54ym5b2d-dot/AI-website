...
- T009 等待 Robert 审核和 Pull Request 合并；T010 在 T009 合并前继续保持 Blocked。
...
1. 审核并合并 GitHub Issue #4 / T009 的实现。
2. T009 通过后执行 T010：素材浏览和详情页，并同步准备腾讯云账号、COS、备案资料和部署环境。

## 5. 当前最高优先级

当前最高优先级是审核 T009 的本地数据库、身份 API、角色守卫和自动化测试。审核通过并合并后进入 T010；腾讯云账号、COS、支付和备案可以并行准备，但真实外部接入统一在 T017 完成。
