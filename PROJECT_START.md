...
- T012 / Issue #7 已由 Robert 验收并通过 PR #32 压缩合并到 `main`（`8e5b42b`），状态为 Done；T012A / Issue #33 已通过草稿 PR #35 进入 Review，远程 `verify` 已通过，等待 Robert 页面验收和合并；T013-T016 继续 Blocked，T017 保持 Backlog。
...
- 交易、下载和收益功能尚未正式开发；后续任务必须延续本地真实 PostgreSQL/Prisma、可替换 provider 和自动化测试，不得回退到无法运行的 mock。
- T012A 当前为 Review：本地邀请码、系统设置、用户只读、上传者个人页、公开认证摘要、法律文档和准确未开放状态已落地；草稿 PR #35 的远程 `verify` 已通过，等待 Robert 页面验收和合并。T013 仍等待 T012A Done，T014-T016 继续按既定前置保持 Blocked。
- Notion 总控页已按 v5.5 同步 PR #35、远程检查与 Project Review 证据，并在写入后重新读取确认。
...
1. Robert 预览并验收 T012A / Issue #33；草稿 PR #35 的 `verify` 已通过，确认页面后再合并并转 Done。
2. T012A 合并完成后把 T013 转为 Ready；真实支付和外部服务接入分别按 T013、T017 的既定边界推进。

## 5. 当前最高优先级

当前最高优先级是验收 GitHub Issue #33 / T012A `后台基础管理与现有入口补全`。本地实现、自动检查及桌面/390px 页面复核已完成，状态为 Review；T013 继续等待 T012A 合并完成。真实外部接入继续统一在 T017 完成。
