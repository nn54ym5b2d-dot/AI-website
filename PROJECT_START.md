...
- GitHub Project #1 已复核：T006-T010、T009A Done，T011 Review，T013 Ready，T012、T014-T016 Blocked，T017 Backlog；T011 已关联草稿 PR #30。
...
- T011 / Issue #6 已在分支 `codex/t011-uploader-submission` 完成本地实现并通过草稿 PR #30 进入 Review：人物独立表单、物件/场景共用表单、草稿创建/编辑、上传意图、暂存到不可变最终 key、重复完成幂等、人物必填/物件场景选填的私有证明材料、独立水印预览/缩略对象元数据、提交后的待支付结果页和提交幂等均已落地；真实 COS、图片处理、CDN 和认证费支付仍未接入。
...
- 后台审核、交易、下载和收益功能尚未正式开发；后续任务必须延续本地真实 PostgreSQL/Prisma、可替换 provider 和自动化测试，不得回退到无法运行的 mock。
- T011 当前等待 Robert 审核分支/草稿 PR；在验收并合并前保持 Review，T012 继续 Blocked。T013 仍为 Ready，但当前先完成 T011 收尾。
...
1. Robert 审核 T011 的页面、代码和草稿 PR；通过后合并并把 T011 转为 Done。
2. T011 合并后将 T012 转为 Ready，继续后台素材审核和认证记录；T013 仍保持 Ready。真实外部接入统一在 T017 完成。

## 5. 当前最高优先级

当前最高优先级是审核 GitHub Issue #6 / T011 `开发上传者素材提交流程` 的本地实现和草稿 PR。只有 Robert 验收并合并后，T011 才转为 Done 并解锁 T012；真实外部接入仍统一在 T017 完成。
